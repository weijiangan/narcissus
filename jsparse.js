...
Tokenizer.prototype = {
  ...
  peek: function() {
    ...
    if (this.lookahead) {
      ...
      if (this.scanNewlines && next.lineno != this.lineno) tt = NEWLINE;
      else tt = next.type;
    } else {
      tt = this.get();
      this.unget();
  ...
  get: function() {
    ...
    for (;;) {
      ...
      if (match) {
        ...
        if (newlines) this.lineno += newlines.length;
        input = this.input;
      }
    ...
    if (!input) return (token.type = END);
    ...
    if ((match = fpRegExp(input))) {
      ...
      if (assignOps[op] && input[op.length] == "=") {
        ...
        if (this.scanOperand && (token.type == PLUS || token.type == MINUS)) {
...
function Node(t, type) {
  ...
  for (var i = 2; i < arguments.length; i++) this.push(arguments[i]);
}
...
Np.__defineGetter__("filename", function() {
...
function Statements(t, x) {
  ...
  x.stmtStack.push(n);
  while (!t.done && t.peek() != RIGHT_CURLY) n.push(Statement(t, x));
  x.stmtStack.pop();
  return n;
}
...
function Statement(t, x) {
  ...
  switch (tt) {
      ...
      while ((tt = t.get()) != RIGHT_CURLY) {
        switch (tt) {
          ...
          case CASE:
            ...
            else n2.caseLabel = Expression(t, x, COLON);
            break;
          default:
    ...
    case CONTINUE:
      ...
      if (label) {
        do {
          if (--i < 0) throw t.newSyntaxError("Label not found");
    ...
    case TRY:
      ...
      while (t.match(CATCH)) {
        ...
        if (t.match(IF)) {
          if (x.ecmaStrictMode) throw t.newSyntaxError("Illegal catch guard");
    ...
    case RETURN:
      if (!x.inFunction) throw t.newSyntaxError("Invalid return");
      n = new Node(t);
      tt = t.peekOnSameLine();
      ...
      if (tt == IDENTIFIER) {
        ...
        if (tt == COLON) {
          ...
          for (i = ss.length - 1; i >= 0; --i) {
            if (ss[i].label == label) throw t.newSyntaxError("Duplicate label");
          }
          t.get();
          n = new Node(t, LABEL);
          n.label = label;
...
function FunctionDefinition(t, x, requireName, functionForm) {
  ...
  while ((tt = t.get()) != RIGHT_PAREN) {
    if (tt != IDENTIFIER) throw t.newSyntaxError("Missing formal parameter");
    f.params.push(t.token.value);
    if (t.peek() != RIGHT_PAREN) t.mustMatch(COMMA);
  }
  ...
  if (functionForm == DECLARED_FORM) x.funDecls.push(f);
  return f;
}
...
for (i in opPrecedence) opPrecedence[GLOBAL[i]] = opPrecedence[i];
...
var opArity = {
  ...
  DELETE: 1,
  VOID: 1,
  TYPEOF: 1, // PRE_INCREMENT: 1, PRE_DECREMENT: 1,
  NOT: 1,
  BITWISE_NOT: 1,
  UNARY_PLUS: 1,
  UNARY_MINUS: 1,
...
for (i in opArity) opArity[GLOBAL[i]] = opArity[i];
...
function Expression(t, x, stop) {
  ...
  function reduce() {
    ...
    for (var i = 0; i < arity; i++) n.push(a[i]);

    // Include closing bracket or postfix operator in [start,end).
    if (n.end < t.token.end) n.end = t.token.end;

    operands.push(n);
    return n;
  }

  loop: while ((tt = t.get()) != END) {
    if (
      tt == stop &&
      x.bracketLevel == bl &&
      x.curlyLevel == cl &&
    ...
    switch (tt) {
      ...
      case COLON:
        ...
        if (tt == COLON) {
          n = operators.top();
          if (n.type != HOOK) throw t.newSyntaxError("Invalid label");
          --x.hookLevel;
        } else {
          operators.push(new Node(t));
          if (tt == ASSIGN) operands.top().assignOp = t.token.assignOp;
          else ++x.hookLevel; // tt == HOOK
        }
        t.scanOperand = true;
        break;
      ...
      case DOT:
        if (t.scanOperand) break loop;
        while (opPrecedence[operators.top().type] >= opPrecedence[tt]) reduce();
        if (tt == DOT) {
          t.mustMatch(IDENTIFIER);
      ...
      case RIGHT_BRACKET:
        if (t.scanOperand || x.bracketLevel == bl) break loop;
        while (reduce().type != INDEX) continue;
        --x.bracketLevel;
        break;
        ...
        object_init: if (!t.match(RIGHT_CURLY)) {
          do {
            ...
            if (
              ...
              t.mustMatch(COLON);
              n.push(new Node(t, PROPERTY_INIT, id, Expression(t, x, COMMA)));
            }
          } while (t.match(COMMA));
          t.mustMatch(RIGHT_CURLY);
        }
        ...
        if (t.scanOperand) {
          ...
          if (t.match(RIGHT_PAREN)) {
            if (n.type == NEW) {
              ...
              n = new Node(t, CALL, operands.pop(), new Node(t, LIST));
            }
            operands.push(n);
            t.scanOperand = false;
            break;
          }
          if (n.type == NEW) n.type = NEW_WITH_ARGS;
          else operators.push(new Node(t, CALL));
        }
        ++x.parenLevel;
        break;

      case RIGHT_PAREN:
        if (t.scanOperand || x.parenLevel == pl) break loop;
        while (
          (tt = reduce().type) != GROUP &&
          tt != CALL &&
          tt != NEW_WITH_ARGS
        ) {
          continue;
        ...
        if (tt != GROUP) {
          n = operands.top();
          if (n[1].type != COMMA) n[1] = new Node(t, LIST, n[1]);
          else n[1].type = LIST;
        }
        --x.parenLevel;
        break;
